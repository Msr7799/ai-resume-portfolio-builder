...
Copilot said: I'll search for files related to "omni"

I'll search for files related to "omni" in the chat-ui repository.الملفات المسؤولة عن استخدام Omni في البرنامج:
ملفات الخادم (Backend):
1. src/lib/server/router/arch.ts

    معالجة منطق التوجيه الذكي باستخدام Arch-Router
    دالة archSelectRoute() لاختيار أفضل نموذج بناءً على السياق
    تحويل المحادثات إلى صيغة موجهة للراوتر

2. src/lib/server/models.ts

    إنشاء نموذج وهمي (alias) للراوتر باسم "Omni"
    تحديد خصائص الراوتر (اسم العرض، الشعار، معرّف النموذج)
    إضافة دعم الصور (Multimodal) والأدوات (Tools) للراوتر
    دالة makeRouterEndpoint() لإنشاء نقطة نهاية الراوتر

3. src/lib/server/router/ (مجلد كامل)

    يحتوي على منطق التوجيه الكامل
    معالجة المسارات والنماذج البديلة

ملفات النوع/البيانات:
4. src/lib/types/Settings.ts

    تخزين تفضيلات المستخدم المتعلقة بالنماذج
    حقول مثل hidePromptExamples وproviderOverrides

واجهة المستخدم (Frontend):
5. src/lib/components/icons/IconOmni.svelte

    أيقونة Omni المعروضة في الواجهة

6. src/lib/components/WelcomeModal.svelte

    شاشة الترحيب التي تشرح وظيفة Omni
    عرض رسالة الترحيب عند أول استخدام

ملفات التكوين:
7. .env و CLAUDE.md

    متغيرات البيئة المتعلقة بالراوتر:
        LLM_ROUTER_ARCH_BASE_URL
        LLM_ROUTER_ROUTES_PATH
        PUBLIC_LLM_ROUTER_DISPLAY_NAME
        PUBLIC_LLM_ROUTER_ALIAS_ID

8. docs/source/configuration/llm-router.md

    توثيق شامل لكيفية تكوين Omni

ملخص: Omni هو نظام توجيه ذكي يختار أفضل نموذج AI تلقائياً بناءً على محتوى المحادثة، ويتم التحكم به من خلال ملفات الراوتر والتكوين والواجهة الرسومية.
Skip to content
...
gary149
gary149
fix: rewrite streaming smoothing algorithm with character-rate targeting
b892d8b
 · 
3 months ago

192 lines (161 loc) · 7.79 KB
